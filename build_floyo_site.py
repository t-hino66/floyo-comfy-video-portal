...
<head>
    ...
    <style>
        :root {
            ...
            --badge-paid-bg: rgba(239, 68, 68, 0.15);
            --badge-paid-border: rgba(239, 68, 68, 0.3);
            --badge-paid-text: #f87171;
            --radius-lg: 16px;
            --radius-md: 10px;
            --radius-sm: 6px;
        }
        ...
        * {
        ...
        .hero h1 {
            ...
            margin-bottom: 1rem;
            background: linear-gradient(135deg, #ffffff 0%, #cbd5e1 50%, var(--accent-glow) 100%);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
        }
        ...
        @media (max-width: 768px) {
            ...
            .stats-bar { flex-direction: column; gap: 0.5rem; border-radius: var(--radius-md); }
        }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="grid" id="card-grid">
            <!-- Dynamic JavaScript cards render here -->
        </div>
    </div>
    ...
    <div class="modal-overlay" id="modal-overlay">
        <div class="modal-card">
            ...
            <div class="category-tag" id="modal-category">CATEGORY</div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
                ...
                if (filtered.length === 0) {
                    cardGrid.innerHTML = `
                        <div style="grid-column: 1 / -1; text-align: center; padding: 4rem; color: var(--text-muted);">
                            条件に一致する動画ワークフロー情報が見つかりませんでした。検索ワードを変更してください。
                        </div>
                    `;
                    return;
                }
            ...
            function openModal(item) {
                ...
                if (item.details) {
                    ...
                    if (item.details.key_tips) {
                        html += `
                            <div class="modal-section-title">制作Tips・ノウハウ</div>
                            <ul style="color: var(--text-secondary); padding-left: 1.2rem; font-size: 0.9rem;">
                                ${item.details.key_tips.map(tip => `<li style="margin-bottom: 0.3rem;">${tip}</li>`).join('')}
                            </ul>
                        `;
                    }
...
def build_site():
    ...
    final_html = HTML_TEMPLATE.replace("__DATABASE_JSON_PLACEHOLDER__", db_json_str)
    
    with open(OUTPUT_HTML, 'w', encoding='utf-8') as f:
        f.write(final_html)
        
    print(f"Successfully generated {OUTPUT_HTML} (Size: {len(final_html)} bytes).", flush=True)
